'use strict';
/**
 * Valentine "Say Yes" generator behaviour: step navigation, draft saving,
 * preview and publish. Field ids here mirror templates/valentine-say-yes/schema.js
 * exactly.
 */
(function () {
  var FIELDS = ['recipientName', 'senderName', 'question', 'yesLabel', 'noLabel', 'celebration'];
  var LAST_STEP = 3;
  ...
  function renderReview() {
    ...
    var rows = [
      ...
      ['The question', data.question || 'Will you be my Valentine?'],
      ['The yes button', data.yesLabel || 'Yes'],
      ['The no button', data.noLabel || 'No'],
      ['The celebration', data.celebration || 'Yayyy!! :3'],
  ...
  var MESSAGES = {
    ...
    limit: 'That is a lot of questions for one day. Try again a bit later.',
    storage_unavailable: 'Publishing is paused right now. Your draft is safe — try again shortly.',
    forbidden: 'This question can’t be edited any more.',
    not_found: 'This question has wandered off. Reload and start again.',
  };
  ...
  function saveDraft() {
    ...
    saving = true;
    say('Saving…');
    return request('/api/valentine-say-yes/draft', { id: draftId, customer_data: collect() })
      .then(function (body) {
        draftId = body.id;
  ...
  if (previewBtn) {
    previewBtn.addEventListener('click', function () {
      ...
      saveDraft().then(function () {
        if (previewState) previewState.textContent = 'Saved. Your question is ready to send.';
  ...
  if (publishBtn) {
    publishBtn.addEventListener('click', function () {
      ...
      say('Publishing…');
      request('/api/valentine-say-yes/publish', { id: draftId })
