'use strict';
/**
 * Saalgirah generator behaviour: step navigation, draft saving, preview and
 * publish. Field ids here mirror templates/saalgirah/schema.js exactly.
 */
(function () {
  ...
  function show(index) {
    ...
    if (counter) counter.textContent = 'Step 0' + (step + 1) + ' of 06';
  ...
  function renderReview() {
    if (!review) return;
    var data = collect();
    var name = data.recipientName || 'them';
    var rows = [
      ['For', data.recipientName],
      ['First line', data.line1 || 'I was going to write something normal…'],
      ['Second line', data.line2 || 'but you’re not exactly a normal person to me.'],
      ['Third line', data.line3 || 'So… I made you this.'],
      ['Before the cake', data.attentionLine || 'Okay… now that I have your attention.'],
      ['The wish', data.wishLine || 'HAPPY BIRTHDAY, ' + name.toUpperCase() + '.'],
      ['The last words', data.closingLine || 'I LOVE YOU.'],
      ['Note', data.note],
  ...
  function saveDraft() {
    ...
    saving = true;
    say('Saving…');
    return request('/api/saalgirah/draft', { id: draftId, customer_data: collect() })
      .then(function (body) {
        draftId = body.id;
  ...
  if (previewBtn) {
    previewBtn.addEventListener('click', function () {
      ...
      saveDraft().then(function () {
        if (previewState) previewState.textContent = 'Saved. Your letter is ready to send.';
