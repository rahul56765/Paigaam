'use strict';
/**
 * Love Album generator behaviour: step navigation, photo preparation and
 * upload, draft saving, preview and publish. Field ids mirror
 * templates/love-album/schema.js exactly. Photos are re-encoded client-side
 * via canvas (long edge ≤1600px, JPEG q0.82) before upload — the server
 * re-validates magic bytes and dimensions.
 */
(function () {
  ...
  function renderReview() {
    ...
    var rows = [
      ...
      ['Photos', String(data.photos.length) + ' uploaded'],
      ['The intro', data.introLine],
  ...
  function renderPhotos() {
    if (photoCount) photoCount.textContent = photos.length ? photos.length + ' of ' + MAX_PHOTOS + ' photos' + (photos.length < MIN_PHOTOS ? ' — need at least ' + MIN_PHOTOS : '') : '';
    if (!photoList) return;
    photoList.innerHTML = '';
  ...
  var MESSAGES = {
    ...
    limit: 'That is a lot of albums for one day — try again a bit later.',
    too_few_photos: 'The gallery needs at least 3 photos before it can go live.',
  ...
  function saveDraft() {
    ...
    saving = true;
    say('Saving…');
    return request('/api/love-album/draft', { id: draftId, customer_data: collect() })
      .then(function (body) {
        draftId = body.id;
  ...
  function gatePublish() {
    ...
    var ready = draftId && photos.filter(function (p) { return p.url; }).length >= MIN_PHOTOS && photos.every(function (p) { return p.url; });
  ...
  if (previewBtn) {
    previewBtn.addEventListener('click', function () {
      ...
      if (photos.length < MIN_PHOTOS) { setError('Choose at least ' + MIN_PHOTOS + ' photos — step 03.'); show(2); return; }
      ...
      saveDraft().then(function () {
        ...
        pending.forEach(function (photo) {
          uploadChain = uploadChain.then(function () {
            return request('/api/love-album/upload?id=' + encodeURIComponent(draftId), null, photo.blob, 'image/jpeg').then(function (result) {
        ...
        busy = false;
        if (previewState) previewState.textContent = 'Saved. Your album is ready to send.';
        gatePublish();
        openPreview();
      }).catch(function (err) {
        busy = false;
