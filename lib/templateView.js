...
const lavender = require('./lavenderRoutes');
const courtyard = require('./courtyardRoutes');
const maafi = require('./maafiRoutes');
const loveAwaits = require('./loveAwaitsRoutes');
const sawaal = require('./sawaalRoutes');
const valentine = require('./valentineRoutes');
const love = require('./loveRoutes');
const sauwajah = require('./sauwajahRoutes');
const bfday = require('./bfdayRoutes');
...
function templateSampleView(tpl, baseUrl) {
  if (bfday.has(tpl.slug)) {
    return renderPaigaamPage(
      { slug: tpl.slug, category: tpl.category, config: tpl.config },
      { customer_data: bfday.demo(tpl.slug), slug: null },
      { baseUrl, isPreview: true });
  }
  if (tpl.slug === love.SLUG) {
    return renderPaigaamPage(
      { slug: tpl.slug, category: tpl.category, config: tpl.config },
      { customer_data: { recipientName: 'Meher', senderName: 'Rahul', photos: love.demoSample ? love.demoSample() : [] }, slug: null },
      { baseUrl, isPreview: true });
  }
  if (tpl.slug === maafi.SLUG) {
    return renderPaigaamPage(
      { slug: tpl.slug, category: tpl.category, config: tpl.config },
      { customer_data: maafi.DEMO, slug: null },
      { baseUrl, isPreview: true });
  }
  if (tpl.slug === sauwajah.SLUG) {
    return renderPaigaamPage(
      { slug: tpl.slug, category: tpl.category, config: tpl.config },
      { customer_data: { recipientName: 'Meher', senderName: 'Rahul', photos: sauwajah.demoSample().slice(0, 4) }, slug: null },
      { baseUrl, isPreview: true });
  }
  if (tpl.slug === loveAwaits.SLUG) {
    return renderPaigaamPage(
      { slug: tpl.slug, category: tpl.category, config: tpl.config },
      { customer_data: loveAwaits.DEMO, slug: null },
      { baseUrl, isPreview: true });
  }
  if (tpl.slug === sawaal.SLUG) {
    return renderPaigaamPage(
      { slug: tpl.slug, category: tpl.category, config: tpl.config },
      { customer_data: sawaal.DEMO, slug: null },
      { baseUrl, isPreview: true });
  }
  if (tpl.slug === valentine.SLUG) {
    return renderPaigaamPage(
      { slug: tpl.slug, category: tpl.category, config: tpl.config },
      { customer_data: valentine.DEMO, slug: null },
      { baseUrl, isPreview: true });
  }
  if (tpl.slug === lavender.SLUG) {
    return renderPaigaamPage(
      { slug: tpl.slug, category: tpl.category, config: tpl.config },
      { customer_data: lavender.DEMO, slug: null },
      { baseUrl, isPreview: true });
  }
