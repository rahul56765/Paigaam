'use strict';
// Restores Valentine "Say Yes" media from the checksummed base64 sources at boot.
// Same self-healing pattern as lib/courtyardMedia.js: the git tree only ever
// carries base64 text (binary-through-JSON git pushes get U+FFFD-corrupted);
// the browser always receives ordinary GIF bytes.
const fs = require('node:fs');
const path = require('node:path');
const crypto = require('node:crypto');
const SOURCE = path.join(__dirname, '../assets/valentine');
const DEST = path.join(__dirname, '../public/valentine-say-yes/media');

function ensureValentineMedia() {
  let manifest;
  ...
  for (const [name, expected] of Object.entries(manifest)) {
    if (!/^valentine-[1-7]\.gif$/.test(name)) throw new Error('invalid media filename: ' + name);
...
module.exports = { ensureValentineMedia };
