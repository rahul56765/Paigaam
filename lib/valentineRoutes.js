'use strict';
/**
 * Valentine "Say Yes" routes — draft, preview, publish.
 *
 * Same shape as the Lavender Bloom module: a creator-cookie ownership table,
 * drafts in the shared paigaams table, free instant publishing. No uploads,
 * no media handling, no languages — the only state this template owns is
 * valentine_owners.
 */
const crypto = require('node:crypto');
const { db, q, PERSISTENT } = require('../db');
const { validate, InputError } = require('../templates/valentine-say-yes/schema');
const { renderValentine } = require('../templates/valentine-say-yes/render');
const { valentineCreatePage } = require('../pages/valentineCreate');

// Additive table: nothing existing is touched.
db.exec(`CREATE TABLE IF NOT EXISTS valentine_owners (
 paigaam_id TEXT PRIMARY KEY REFERENCES paigaams(id) ON DELETE CASCADE,
 owner_hash TEXT NOT NULL, created_at INTEGER NOT NULL);
CREATE INDEX IF NOT EXISTS valentine_owner_idx ON valentine_owners(owner_hash);`);

const SLUG = 'valentine-say-yes';
...
function owned(req, id) {
  const record = db.prepare('SELECT owner_hash FROM valentine_owners WHERE paigaam_id=?').get(id);
...
/** A believable question for /valentine-say-yes/demo and the collection thumbnails. */
const DEMO = {
  recipientName: 'Meher',
  senderName: 'Rahul',
...
async function handle(req, res, u, { baseUrl, isAdmin = false } = {}) {
  ...
  const handles = p === '/create/' + SLUG || p === '/valentine-say-yes/demo' ||
    p.startsWith('/valentine-say-yes/preview/') || p.startsWith('/api/valentine-say-yes/');
  ...
  try {
    ...
    if (req.method === 'GET' && p === '/create/' + SLUG) {
      ...
      html(res, valentineCreatePage());
      return true;
    }

    if (req.method === 'GET' && p === '/valentine-say-yes/demo') {
      html(res, renderValentine({ customer_data: DEMO }, { baseUrl, isPreview: true }));
      return true;
    }

    let m = p.match(/^\/valentine-say-yes\/preview\/([A-Za-z0-9_-]+)$/);
    if (req.method === 'GET' && m) {
      const pg = q.paigaamById(m[1]);
      if (!pg || pg.template_slug !== SLUG) throw new InputError('not_found', 404);
      if (!isAdmin && !owned(req, pg.id)) throw new InputError('forbidden', 403);
      html(res, renderValentine(pg, { baseUrl, isPreview: true }));
      return true;
    }

    if (req.method === 'POST' && p === '/api/valentine-say-yes/draft') {
      ...
      if (id) {
        ...
        const count = db.prepare('SELECT COUNT(*) n FROM valentine_owners WHERE owner_hash=? AND created_at>?').get(hash, Date.now() - 86400000).n;
        ...
        id = pg.id;
        db.prepare('INSERT INTO valentine_owners VALUES(?,?,?)').run(id, hash, Date.now());
      }
      reply(res, 200, { id, previewUrl: '/valentine-say-yes/preview/' + id });
      return true;
    }

    if (req.method === 'POST' && p === '/api/valentine-say-yes/publish') {
      ...
      const allowEphemeral = process.env.VALENTINE_ALLOW_EPHEMERAL_PUBLISH === '1';
      ...
      let slug;
      do { slug = 'valentine-' + crypto.randomBytes(9).toString('hex'); } while (q.paigaamSlugTaken(slug));
    ...
    if (!(e instanceof InputError)) console.error('[valentine-say-yes]', e.message);
