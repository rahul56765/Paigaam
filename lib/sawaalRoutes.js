...
function cleanAnswer(scene, answer) {
  ...
  if (answer.date !== undefined) {
    // null = the sender offered no days, the picker was skipped
    if (answer.date === null) { clean.date = null; return clean; }
    clean.date = s(answer.date);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(clean.date || '')) throw new InputError();
  }
