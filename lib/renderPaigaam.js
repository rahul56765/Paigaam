...
function renderPaigaamPage(tpl, paigaam, opts = {}) {
  ...
  if (tpl.slug === 'saalgirah') return require('../templates/saalgirah/render').renderLetter(paigaam, opts);
  if (tpl.slug === 'lavender-tic-tac-toe-bloom') return require('../templates/lavender-bloom/render').renderBloom(paigaam, opts);
  if (tpl.slug === 'ganpati-courtyard') return require('../templates/ganpati-courtyard/render').renderVideoPage(paigaam, opts);
  if (tpl.slug === 'maafi') return require('../templates/maafi/render').renderMaafi(paigaam, opts);
  if (tpl.slug === 'love-awaits') return require('../templates/love-awaits/render').renderAwaits(paigaam, opts);
  if (tpl.slug === 'sawaal') return require('../templates/sawaal/render').renderSawaal(paigaam, opts);
  if (tpl.slug === 'valentine-say-yes') return require('../templates/valentine-say-yes/render').renderValentine(paigaam, opts);
  if (tpl.slug === 'love-album') return require('../templates/love-album/render').renderAlbum(paigaam, opts);
  if (tpl.slug === 'sau-wajah') return require('../templates/sau-wajah/render').renderSauWajah(paigaam, opts);
  // Boyfriend Day family: every registered member renders itself (lib/bfday/family.js).
  const bfdayMember = require('./bfday/family').bySlug(tpl.slug);
  if (bfdayMember) return bfdayMember.render(paigaam, opts);
