...
async function handle(req,res,u,{baseUrl,isAdmin=false}={}){
 ...
 try{
  ...
  if(req.method==='POST'&&p==='/api/ganapati/publish'){
   ...
   // The owner accepted (2026-09-05) that publishing stays blocked on ephemeral
   // storage; GANAPATI_ALLOW_EPHEMERAL_PUBLISH=1 is the explicit escape hatch.
   const allowEphemeral = process.env.GANAPATI_ALLOW_EPHEMERAL_PUBLISH === '1';
