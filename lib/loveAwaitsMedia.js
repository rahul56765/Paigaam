'use strict';
// Restores Love Awaits media from the checksummed base64 sources at boot.
...
const SOURCE = path.join(__dirname, '../assets/love-awaits');
const DEST = path.join(__dirname, '../public/love-awaits/media');

function ensureLoveAwaitsMedia() {
  let manifest;
  ...
  for (const [name, expected] of Object.entries(manifest)) {
    if (!/^loveawaits-[1-7]\.(gif|webp)$/.test(name)) throw new Error('invalid media filename: ' + name);
...
module.exports = { ensureLoveAwaitsMedia };
