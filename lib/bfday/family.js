...
const { normalizeFields, InputError, BGM_FIELD } = require('./fields');
...
function loadOne(slug, dir) {
  ...
  const fields = normalizeFields([...config.fields, BGM_FIELD], where);
  const ids = new Set(fields.map(f => f.id));

  // Every family template carries the shared optional background-song field
  // (lib/bfday/fields.BGM_FIELD) as the last field of its last wizard step;
  // the engine normalises it to a bare YouTube id before any render sees it.
  if (!config.steps.some(s => Array.isArray(s.fields) && s.fields.includes(BGM_FIELD.id))) {
    config.steps[config.steps.length - 1].fields.push(BGM_FIELD.id);
  }
  ...
  if (config.steps.length > 7) throw new Error(`${where}/config.js: at most 7 steps`);
