...
const TYPES = ['text', 'textarea', 'number', 'url', 'select', 'image', 'date', 'list'];
...
function imagesIn(fields, data) {
  ...
  for (const f of fields) {
    if (f.type === 'image' && typeof d[f.id] === 'string' && UPLOAD_URL.test(d[f.id])) urls.push(d[f.id]); // demo assets are not uploads
...
module.exports = {
  InputError, TYPES, SCALAR, UPLOAD_URL, DEMO_ASSET_URL, IMAGE_URL, CONTROL, isRealDate,
