...
const TYPES = ['text', 'textarea', 'number', 'url', 'select', 'image', 'date', 'list', 'bgm'];
/** A calendar day, YYYY-MM-DD (what <input type="date"> sends). */
 ...
 * A template's designed demo may also point at its shipped demo photos. */
const UPLOAD_URL = /^\/bfday\/uploads\/[a-f0-9]{48}\.(?:jpg|png|webp)$/;
const DEMO_ASSET_URL = /^\/assets\/bfday-demo\/[a-z0-9-]{1,60}\.(?:jpg|png|webp)$/;
const IMAGE_URL = new RegExp('(?:' + UPLOAD_URL.source + ')|(?:' + DEMO_ASSET_URL.source + ')');
...
function normalizeScalar(f, where) {
  ...
  if (f.type === 'bgm') {
    // The shared background-song field: validated by bgmClean, never by the
    // generic scalar rules (a full YouTube URL legitimately contains '?').
    out.maxLength = f.maxLength == null ? 300 : f.maxLength;
    if (!Number.isInteger(out.maxLength) || out.maxLength < 1 || out.maxLength > 2000) throw specError(where, 'bgm maxLength must be an integer 1..2000');
    if (f.default != null && bgmClean(f.default) === '' && String(f.default).trim() !== '' && String(f.default).trim() !== 'none') throw specError(where, 'bgm default must be a YouTube link, an 11-char id, or blank');
  }
  ...
  if (f.type === 'date') {
    if (f.default != null && (typeof f.default !== 'string' || !isRealDate(f.default))) throw specError(where, 'date default must be YYYY-MM-DD');
  }
  if (f.type === 'image') {
...
function scalarValue(f, raw, mode) {
  ...
  if (f.type === 'bgm') {
    // Never throws: 'none'/blank → '', a usable link/id → the bare id,
    // anything else → ''. Music must never block a publish.
    if (raw != null && typeof raw !== 'string') return fail();
    const v = bgmClean(raw);
    return v;
  }
  ...
  if (f.type === 'image' && !IMAGE_URL.test(value)) { const r = fail(); return r === null ? '' : r; }
  if (f.type === 'date' && !isRealDate(value)) { const r = fail(); return r === null ? '' : r; }
  return value;
}
...
/**
 * The one field every family template shares: an optional background song.
 * A YouTube link or a bare 11-char video id; blank (or the exact string
 * 'none') = a quiet page. Normalised to the bare id at every validation pass,
 * so renderers can trust `resolve()`'s output to be an id or ''.
 */
const BGM_FIELD = Object.freeze({
  id: 'bgmSong', type: 'bgm', label: 'Background music', hint: 'A YouTube link or video id. Optional — leave it blank (or choose "no song") for a quiet page. Nothing plays until they tap the music button.',
  required: false, maxLength: 300,
});

/** Normalise a raw bgmSong value: '' → '', 'none' → '', a usable URL/id → the bare id, anything else → ''. */
function bgmClean(value) {
...
function makeSchema(config) {
  const declared = normalizeFields(config.fields, config.slug || 'template');
  const fields = declared.some(f => f.id === 'bgmSong') ? declared : Object.freeze([...declared, BGM_FIELD]);
  ...
  function validate(data, { mode = 'strict' } = {}) {
    ...
    const clean = validateFields(fields, data, mode);
    return { templateVersion: version, ...clean };
  }
...
module.exports = {
  InputError, TYPES, SCALAR, UPLOAD_URL, DEMO_ASSET_URL, IMAGE_URL, CONTROL, isRealDate, BGM_FIELD, bgmClean,
