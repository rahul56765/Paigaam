...
function demoPhotos() {
  // Flat-colour placeholder PNGs, spec-exact and generated with Node's
  // built-in zlib (no encoder dependency). Written once at boot; personal
  // media never ships with the template. Chromium verified these decode.
  ...
  return colors.map((rgb, index) => {
    ...
    fs.writeFileSync(target, solidPNG(400, 533, rgb[0], rgb[1], rgb[2]));
...
async function handle(req, res, u, { baseUrl, isAdmin = false } = {}) {
  ...
  try {
    ...
    m = p.match(/^\/love-album\/demo-media\/(demo-[1-6]\.png)$/);
    if (['GET', 'HEAD'].includes(req.method) && m) {
      demoPhotos(); // ensure generated
      if (!streamFile(req, res, path.join(DATA_DIR, 'love-demo', m[1]), 'image/png', 'public, max-age=86400')) throw new InputError('not_found', 404);
      return true;
    }
