'use strict';
/**
 * Love Awaits routes — draft, preview, publish.
 *
 * Same shape as the Valentine "Say Yes" module: a creator-cookie ownership
 * table, drafts in the shared paigaams table, free instant publishing.
 * No uploads, no media handling — the only state this template owns is
 * love_awaits_owners.
 */
const crypto = require('node:crypto');
const { db, q, PERSISTENT } = require('../db');
const { validate, InputError } = require('../templates/love-awaits/schema');
const { renderAwaits } = require('../templates/love-awaits/render');
const { loveAwaitsCreatePage } = require('../pages/loveAwaitsCreate');

// Additive table: nothing existing is touched.
db.exec(`CREATE TABLE IF NOT EXISTS love_awaits_owners (
 paigaam_id TEXT PRIMARY KEY REFERENCES paigaams(id) ON DELETE CASCADE,
 owner_hash TEXT NOT NULL, created_at INTEGER NOT NULL);
CREATE INDEX IF NOT EXISTS love_awaits_owner_idx ON love_awaits_owners(owner_hash);`);

const SLUG = 'love-awaits';
...
function owned(req, id) {
  const record = db.prepare('SELECT owner_hash FROM love_awaits_owners WHERE paigaam_id=?').get(id);
...
/** A believable proposal for /love-awaits/demo and the collection thumbnails. */
const DEMO = {
  recipientName: 'Meher',
  senderName: 'Rahul',
...
async function handle(req, res, u, { baseUrl, isAdmin = false } = {}) {
  ...
  const handles = p === '/create/' + SLUG || p === '/love-awaits/demo' ||
    p.startsWith('/love-awaits/preview/') || p.startsWith('/api/love-awaits/');
  ...
  try {
    ...
    if (req.method === 'GET' && p === '/create/' + SLUG) {
      ...
      html(res, loveAwaitsCreatePage());
      return true;
    }

    if (req.method === 'GET' && p === '/love-awaits/demo') {
      html(res, renderAwaits({ customer_data: DEMO }, { baseUrl, isPreview: true }));
      return true;
    }

    let m = p.match(/^\/love-awaits\/preview\/([A-Za-z0-9_-]+)$/);
    if (req.method === 'GET' && m) {
      const pg = q.paigaamById(m[1]);
      if (!pg || pg.template_slug !== SLUG) throw new InputError('not_found', 404);
      if (!isAdmin && !owned(req, pg.id)) throw new InputError('forbidden', 403);
      html(res, renderAwaits(pg, { baseUrl, isPreview: true }));
      return true;
    }

    if (req.method === 'POST' && p === '/api/love-awaits/draft') {
      ...
      if (id) {
        ...
        const count = db.prepare('SELECT COUNT(*) n FROM love_awaits_owners WHERE owner_hash=? AND created_at>?').get(hash, Date.now() - 86400000).n;
        ...
        id = pg.id;
        db.prepare('INSERT INTO love_awaits_owners VALUES(?,?,?)').run(id, hash, Date.now());
      }
      reply(res, 200, { id, previewUrl: '/love-awaits/preview/' + id });
      return true;
    }

    if (req.method === 'POST' && p === '/api/love-awaits/publish') {
      ...
      const allowEphemeral = process.env.LOVE_AWAITS_ALLOW_EPHEMERAL_PUBLISH === '1';
    ...
    if (!(e instanceof InputError)) console.error('[love-awaits]', e.message);
