...
The new self-publish endpoint refuses non-local publication when the existing persistence flag is false (override: `GANAPATI_ALLOW_EPHEMERAL_PUBLISH=1`, accepted by the owner 2026-09-05). This is an extra safeguard, NOT proof that a configured path is a real persistent mount. Local restart tests are not a production durability claim.
